# TracePilot

[![CI](https://github.com/xypw/tracepilot/actions/workflows/ci.yml/badge.svg)](https://github.com/xypw/tracepilot/actions/workflows/ci.yml)
[![Release](https://img.shields.io/github/v/release/xypw/tracepilot)](https://github.com/xypw/tracepilot/releases/tag/v1.0.0)

TracePilot 是一个面向 Java 测试失败的受控修复 Agent。真实模型模式先在隔离副本中搜索、阅读源码并尝试补丁；测试反馈决定下一步调查或修订。只有隔离试修通过后，系统才展示最终 diff 等待人工确认；确认后在正式工作区再次校验文件指纹、写入并运行定向测试，失败时回滚。

模型每轮只能选择受限的 `search`、`read`、`patch` 或 `stop` 动作；搜索、读取、隔离试修、路径校验、确认绑定、正式写入、测试、回滚和幂等回执都由确定性代码执行。离线模式仍使用固定规则规划器，仅用于安全流程回归。

## 解决的问题

- 测试日志很长，开发者需要先定位可能出错的源码。
- 模型要根据源码和隔离测试反馈调整调查路径，且不能未经确认修改正式仓库。
- 确认后源文件可能已变化，旧补丁必须失效。
- 测试失败时不能留下半成品修改。
- 服务重启后要找回待确认任务；重复确认不能重复执行。

## 工作流

```mermaid
flowchart LR
    A[测试失败日志] --> B[定位候选 Java 文件]
    B --> C[复现白名单定向测试]
    C -- 未复现 --> K[停止修复]
    C -- 已复现 --> D[模型选择搜索或阅读]
    D --> E[隔离副本尝试单文件补丁]
    E --> F{隔离测试通过?}
    F -- 否且未达上限 --> D
    F -- 否且达上限 --> K
    F -- 是 --> G[展示最终 diff 并绑定源码指纹]
    G --> H{人工确认}
    H -- 拒绝 --> K
    H -- 通过 --> I[正式工作区受限写入]
    I --> J[再次运行定向测试]
    J -- 通过 --> L[保存完成回执]
    J -- 失败 --> M[回滚正式源码]
```

LangGraph 负责审批和执行状态，SQLite 保存检查点与幂等回执；FastAPI 提供启动、查询和确认接口。真实模型模式最多调查 12 步、隔离试修 3 次。模型只提出结构化动作，没有直接文件写入或命令执行能力；工具在受限副本运行测试。

## 安全边界

- 只接受仓库内相对路径，拒绝 `..`、绝对路径、符号链接和 Windows 重解析点。
- 只读取允许的文本类型，只修改 `src/main/java/**/*.java`。
- 测试目标必须是 Java 类名格式，测试执行不经过 Shell。
- 模型只访问文件白名单；试修副本不复制 `.git`、环境文件或其他不允许读取的仓库内容。
- 补丁同时绑定源文件 SHA-256 与提案 SHA-256；确认后源文件变化则拒绝执行。
- 测试失败自动恢复原文件；执行回执状态不明确时停止自动重试并要求人工检查。

## 本地运行

要求 Python 3.11+ 和 JDK 17+。

```powershell
python -m venv .venv
.\.venv\Scripts\python.exe -m pip install -r requirements.txt
$env:TRACEPILOT_WORKSPACE_ROOT = "C:\path\to\java-project"
$env:TRACEPILOT_STATE_DIR = "C:\path\to\tracepilot-state"
$env:TRACEPILOT_JAVA = "C:\path\to\java.exe"
$env:TRACEPILOT_JAVAC = "C:\path\to\javac.exe"
.\.venv\Scripts\python.exe -m uvicorn tracepilot.main:app --port 8020
```

打开 `http://127.0.0.1:8020/docs` 查看 FastAPI 接口文档。默认 `TRACEPILOT_PLANNER=offline`，用于无外部 API 的可复现演示。
打开 `http://127.0.0.1:8020/demo` 可在页面中提交失败日志、查看 diff，并选择确认或拒绝。

也可以使用 Docker Compose 启动内置的虚构 Java 故障样例：

```powershell
docker compose up --build
```

真实模型模式还需要：

```powershell
$env:TRACEPILOT_PLANNER = "model"
$env:TRACEPILOT_MODEL_BASE_URL = "https://provider.example/v1"
$env:TRACEPILOT_MODEL_API_KEY = "your-key"
$env:TRACEPILOT_MODEL = "your-model"
```

不要把密钥写入仓库。发送到模型的内容包含失败日志、模型主动读取的允许文件片段及隔离测试输出，使用真实业务代码前应完成数据分级和供应商授权。

## API

- `POST /runs`：提交失败日志与白名单测试类，生成待确认补丁。
- `GET /runs/{run_id}`：查询状态、补丁、diff、工具轨迹和测试结果。
- `POST /runs/{run_id}/approval`：提交 `approved` 与补丁摘要；摘要必须与当前提案一致。

## 测试与评测

```powershell
.\.venv\Scripts\python.exe -m pytest
.\.venv\Scripts\python.exe -m scripts.run_evaluation `
  --java "C:\path\to\java.exe" `
  --javac "C:\path\to\javac.exe" `
  --output reports\offline-evaluation.json
```

固定评测集包含 12 类 Java 故障：空指针、边界条件、整数除法、字符串引用比较、BigDecimal 精度语义、空集合、循环越界、大小写、布尔条件和整数溢出等。每个案例都从原始夹具复制到临时目录，因此评测不会修改夹具。

当前离线报告见 [`reports/offline-evaluation.json`](reports/offline-evaluation.json)：12/12 原始案例先失败；12/12 定位并提出预期文件补丁；12/12 确认前零写入、重启后恢复、确认后测试通过，重复确认保持幂等。这里使用的是确定性规则规划器，**不能解释为真实大模型修复成功率**。

真实模型评测入口为 `scripts/run_model_evaluation.py`。它会把失败日志、模型主动读取的源码和试修测试输出发送给配置的模型供应商，必须先确认数据授权；报告记录模型、地址、逐例结果和延迟，但不会记录 API Key。

2026-09-15 获得虚构数据外发授权后，对 GLM-4.7-Flash 发起固定案例评测。第一批 12 条因 Windows 下 SQLite 连接未在临时目录清理前释放而全部成为评测器错误；修复资源关闭顺序后，1 条冒烟请求又收到供应商 HTTP 429，因此仍没有可用于统计模型准确率的有效样本。脱敏汇总见 [`reports/model-evaluation-summary-zhipu-20260915.json`](reports/model-evaluation-summary-zhipu-20260915.json)；含本机临时路径的原始排错报告仅本地保留且被 Git 忽略。这些结果只用于暴露评测基础设施和外部依赖问题，**不作为模型修复准确率，也不写入简历成果**。

GitHub Actions 会运行 Python 测试、12 类离线端到端评测并构建 Docker 镜像，阻止工作流、安全边界、修复行为或容器构建回归。

2026-09-23 本地 Python 回归 26/26 通过，其中新增路径使用固定模型动作回放，并在一项测试中运行真实 `javac/java` 验证隔离试修、审批与正式执行。12 类固定 Java 故障的离线规则规划器评测重新运行，12/12 任务成功、定向测试通过、确认前零写入、检查点恢复及重复确认幂等。新模型调查路径仍未进行外部真实模型批量评测，不能把规则基线或动作回放写成模型准确率。

同日获准的 1 条虚构故障真实模型冒烟返回供应商 HTTP 429，图安全地结束为 `MODEL_UNAVAILABLE`，无有效补丁样本；报告见 [`reports/model-smoke-20260923.json`](reports/model-smoke-20260923.json)。因此仍无新增 Agent 调查路径的真实模型成功率。

## 简历证据索引

2026-09-24 完成 30/30 本机 Python 回归，并对本机离线服务验证审批阻断、错误提案拒绝、真实 Java 测试及重复批准不再写入。步骤与范围见 [部署验收](reports/deployment-20260924.md)。

| 表述 | 证据 | 限制 |
| --- | --- | --- |
| 12 类故障基线全部修复 | `evaluation_data/cases.json`、`reports/offline-evaluation.json` | 确定性规划器，不是模型成功率 |
| 确认前零写入、重启恢复、重复确认幂等均为 12/12 | 同一逐例报告 | 固定虚构故障集 |
| 30 项 Python 测试通过 | `tests/`、GitHub Actions 与 2026-09-24 部署报告 | 包含固定模型动作回放和真实 Java 测试，不是外部模型成功率 |
| OpenAI 兼容模型入口 | `providers.py`、`run_model_evaluation.py` | 真实指标必须实际外部评测后再写 |

## 项目边界

- 临时目录仅隔离试修文件，不是操作系统安全沙箱。仅运行可信演示仓库；测试代码仍可能访问本机网络和文件，不能接收陌生仓库直接执行。
- 执行器只接受白名单内测试类，使用固定参数列表启动子进程，不接受模型提供任意 Shell。子进程只继承必要环境变量，不继承模型凭据、`JAVA_TOOL_OPTIONS`、`MAVEN_OPTS` 或 `CLASSPATH`；Javac 禁用隐式注解处理。这些措施不能替代容器权限、网络与文件系统隔离。
- 2026-09-24 小范围验证：执行器、环境边界与模型动作回放相关 14 项测试通过；没有新增外部真实模型修复成功率。
- 当前真实模型调查路径使用单模块 Javac 主类测试与单文件补丁，尚未覆盖多模块构建图和多文件联合修复。
- 真实模型接口与批量评测入口已实现；外部模型结果必须在获得明确数据授权并实际运行后才能作为项目指标。
- SQLite 适合个人演示；多实例服务应改用共享检查点与分布式执行回执。
